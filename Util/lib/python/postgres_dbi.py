...
class Database(object):
    ...
    def __init__(self, gusConfigFile):
        ...
        self.gusConfigFile = (
            os.environ["GUS_HOME"] + "/config/gus.config"
    ...
    def named_cursor(self, name, cursorFactory=None, withhold=True):
        ...
        if cursorFactory == "DictCursor":
            return self.dbh.cursor(
                name=name, cursor_factory=psycopg2.extras.DictCursor, withhold=withhold
            )
        if cursorFactory == "RealDictCursor":
            return self.dbh.cursor(
                name=name,
    ...
    def connect(self):
        ...
        self.connectionString = (
            self.connectionString + " password='" + self.__password + "'"
        )
        self.connectionString = (
            self.connectionString + " dbname='" + self.dsnConfig["dbname"] + "'"
        )
        if "host" in self.dsnConfig:
            self.connectionString = (
                self.connectionString + " host='" + self.dsnConfig["host"] + "'"
            )
        if "port" in self.dsnConfig:
            self.connectionString = (
                self.connectionString + " port='" + self.dsnConfig["port"] + "'"
            )
