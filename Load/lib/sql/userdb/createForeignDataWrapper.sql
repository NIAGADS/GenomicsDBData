-- first run the following on the user database

/*
GRANT USAGE ON SCHEMA wdkuser TO genomicsdb; 
GRANT SELECT ON ALL TABLES IN SCHEMA wdkuser TO genomicsdb;

GRANT USAGE ON SCHEMA wdkuser TO <other user>; 
GRANT SELECT ON ALL TABLES IN SCHEMA wdkuser TO <other user>;
*/
...
-- ALTER SERVER wdk_userDB VERSION 'production' OPTIONS (SET host 'localhost', SET port '<port>', SET dbname '<>');
-- also note that since sequences are created locally, they don't get synced.  will need to start sequences from max value in tables
...
CREATE EXTENSION postgres_fdw;
 ...
 -- CREATE FOREIGN DATA WRAPPER wdk_user_fdw;

CREATE SERVER wdk_userDB
        FOREIGN DATA WRAPPER postgres_fdw
        OPTIONS (host 'localhost', port '<port>', dbname '<userdb_name>_<dbversion>');
...
/* sequences must be recreated locally */
