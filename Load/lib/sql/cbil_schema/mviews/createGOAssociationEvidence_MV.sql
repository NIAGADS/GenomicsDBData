...
CREATE MATERIALIZED VIEW CBIL.GOAssociationEvidence AS (
    ...
    mapped_evidence AS (
        SELECT e.gene_id,
            e.go_term_id,
            CASE
                WHEN e.json->>'qualifier' = 'NOT|contributes_to' THEN 'does not contribute to'
            ...
            jsonb_build_object(
                ...
                'text',
                'tooltip',
                (e.json->>'evidence_code')::text || ': ' || ot.name,
                'value',
                e.json->>'go_evidence_code'
            ) AS evidence_code,
            CASE
                WHEN e.json->>'citation' LIKE 'PMID%' THEN jsonb_build_object(
                    'type',
                    'link',
                    'url',
                    '+PUBMED_URL+' || split_part (e.json->>'citation', ':', 2),
                    'value',
                    e.json->>'citation'
                )
                WHEN e.json->>'citation' LIKE 'GO_REF%' THEN jsonb_build_object(
                    'type',
                    'text',
                    'tooltip',
                    REPLACE(
                    ...
                    'value',
                    e.json->>'citation' || ': ' || gref.title
                )
                WHEN e.json->>'citation' LIKE 'Reactome%' THEN jsonb_build_object(
                    'type',
                    'link',
                    'url',
                    '+REACTOME_PATHWAY_URL+' || split_part(e.json->>'citation', ':', 2),
                    'value',
                    e.json->>'citation'
                )
                WHEN e.json->>'citation' LIKE 'DOI%' THEN jsonb_build_object(
                    'type',
                    'link',
                    'url',
                    '+DOI_URL+' || split_part (e.json->>'citation', ':', 2),
                    'value',
                    LOWER(e.json->>'citation')
                )
            END AS citation
        FROM SRes.OntologyTerm ot,
            evidence_json e
            LEFT OUTER JOIN SRes.BibliographicReference gref ON e.json->>'citation' = gref.source_id
        WHERE ot.source_id = REPLACE(e.json->>'evidence_code', ':', '_')
    ),
    ...
    SELECT ga.gene_id,
        ...
        goa.go_term_id,
        CASE
            WHEN pot.name = 'biological_process' THEN 'BP'
            WHEN pot.name = 'molecular_function' THEN 'MF'
            ELSE 'CC'
        END AS ontology,
        ...
        jsonb_build_object(
            ...
            'link',
            'url',
            '+AMIGO_URL+' || REPLACE(ot.source_id, '_', ':') || '#display-lineage-tab',
            'value',
            REPLACE(ot.source_id, '_', ':')
        ) AS go_accession,
        jsonb_build_object(
            'type',
            'text',
        ...
        jsonb_build_object(
            ...
            'table',
            'value',
            ga.gene_symbol || ' ' || string_agg(DISTINCT fej.json->>'association_qualifier', '|'),
            'data',
            jsonb_agg(DISTINCT fej.json),
            'downloadable',
            false,
            'attributes',
            jsonb_build_array(
                jsonb_build_object(
                    ...
                    'string',
                    'help',
                    'explicit relationship between the gene and the GO term or a flag that modifies the interpretation of an association'
                ),
                jsonb_build_object(
                    'name',
                    'evidence_code',
                    ...
                    'json_text',
                    'help',
                    'mouse over for fuller description from the Evidence and Conclusion Ontology (ECO)'
                ),
                jsonb_build_object(
                    'name',
                    'citation',
                    ...
                    'json_text_or_link',
                    'help',
                    'a single reference (publication, data resource, or protocol) in support of the annotation'
                ),
                jsonb_build_object(
                    'name',
                    'annotation_source',
