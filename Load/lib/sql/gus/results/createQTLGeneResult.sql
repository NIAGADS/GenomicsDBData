...
CREATE TABLE Results.QTLGene (
        ...
        NUM_QTLS_IN_GENE INTEGER NOT NULL,
...
--ALTER TABLE Results.QTLGene ADD CONSTRAINT CONST_QTL_PROTOCOL_APP_NODE_ID FOREIGN KEY (protocol_app_node_id) REFERENCES Study.ProtocolAppNode (protocol_app_node_id);
--ALTER TABLE Results.QTLGene ADD CONSTRAINT CONST_QTL_TARGET_ID FOREIGN KEY (target_id) REFERENCES DOTS.Gene (source_id);

-- INDEXES

CREATE INDEX QG_RESULT_PAN ON Results.QTLGene (PROTOCOL_APP_NODE_ID);
...
CREATE INDEX QG_RESULT_VRPK_NL10P ON Results.QTLGene(VARIANT_RECORD_PRIMARY_KEY, NEG_LOG10_PVALUE DESC);
CREATE INDEX QG_RESULT_BIN_INDEX ON Results.QTLGene USING GIST(BIN_INDEX);
CREATE INDEX QG_RESULT_PAN_TOP ON Results.QTLGene(protocol_app_node_id, neg_log10_pvalue desc);
CREATE INDEX QG_RESULT_PAN_RANK ON Results.QTLGene(protocol_app_node_id, rank desc);
