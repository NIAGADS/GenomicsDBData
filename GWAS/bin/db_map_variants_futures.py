...
def parallel_db_lookup(lookups, debug=False, firstHitOnly=True, checkAltVariants=True):
    ...
    try:
        annotator = VariantRecord(debug=debug)
...
def run(header, lookups, chunkSize, debug = False, checkAltVariants=True, append=False):
    ...
    with open(args.mappedFile, writeType) as mfh, \
        ...
        ProcessPoolExecutor(args.maxWorkers) as executor:
        ...
        futureUpdate = {executor.submit(parallel_db_lookup,
            ...
            checkAltVariants=checkAltVariants)
                : c for c in chunks}
